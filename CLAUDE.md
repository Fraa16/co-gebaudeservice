...
- **Logo**: done, and no longer a placeholder — the client delivered final vector
  artwork. The four source files live in `brand/` (see `brand/README.md`); everything
  under `public/` is generated from them by `scripts/build-brand-assets.mjs`, which only
  crops, namespaces ids and rasterises. It never draws or recolours: if the mark is
